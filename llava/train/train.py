...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            video_folder = self.data_args.video_folder
            video_subfolder = self.list_data_dict[i]["video"]
            image_paths = []
            for root, _, files in os.walk(os.path.join(video_folder, video_subfolder)):
                for file in files:
            ...
            try:
                ...
                if "video_time" in self.list_data_dict[i]:
                    video_time = self.list_data_dict[i]["video_time"]
                    frame_time = self.list_data_dict[i]["frame_time"]
                    num_frames_to_sample = len(frame_time)
                else:
                    fps = 10
                    num_frames_to_sample = len(image_paths)
                    frame_list= list(range(1, num_frames_to_sample + 1))
                    frame_time = [frame * (1 / fps) for frame in frame_list]
                    video_time = num_frames_to_sample / fps
                    if '<video>\n' in sources[0]["conversations"][0]["value"]:
                        sources[0]["conversations"][0]["value"].replace('<video>\n', '<image>\n')
                ...
                if self.data_args.add_time_instruction:
                    time_instruciton = f"The video lasts for {video_time:.2f} seconds, and {num_frames_to_sample} frames are uniformly sampled from it. These frames are located at {frame_time}.Given a series of video frames with timestamps, identify the timestamp of the frame that is most relevant to answering the question:"
                    sources[0]["conversations"][0]["value"] = f'{DEFAULT_IMAGE_TOKEN}\n{time_instruciton}\n{sources[0]["conversations"][0]["value"].replace(DEFAULT_IMAGE_TOKEN, "")}'
                
                image = [(image_tensors, int(image_tensors.size()[1] * image_tensors.size()[2] * image_tensors.size()[3]), "video")]
                ...
                print(f"Error: {e}")
                print(f"Failed to read video file: {video_subfolder}")
