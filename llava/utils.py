...
# def process_video_with_frames(folder_path, data_args):
#     # 获取所有帧图片的路径并排序
#     frame_files = sorted(
...
#         raise ValueError("帧图片文件夹为空，请提供有效的帧图片。")
    
#     # 视频总时长（秒）
#     video_time = total_frame_num / data_args.video_fps
    
#     # 计算采样间隔
#     avg_fps = round(data_args.video_fps / data_args.sample_fps)
#     frame_idx = [i for i in range(0, total_frame_num, avg_fps)]
#     frame_time = [i / data_args.sample_fps for i in frame_idx]
...
#             uniform_sampled_frames = np.linspace(0, total_frame_num - 1, data_args.frames_upbound, dtype=int)
#             frame_idx = uniform_sampled_frames.tolist()
#             frame_time = [i / data_args.video_fps for i in frame_idx]
...
#     return sampled_frames, video_time, frame_time, num_frames_to_sample
